...
synchronies.df %>% 
  filter(dist!=0) %>% 
  ggplot()+aes(dist, synch.w)+geom_point()+geom_smooth()
```
...
NAOseason %>% 
  #filter(Season=='Winter') %>% 
  ggplot()+aes(winter.year, mean.nao)+geom_path()+facet_wrap(~Season)+theme_bw()+xlab(NULL)+geom_hline(yintercept = 0, col='grey20')
```
