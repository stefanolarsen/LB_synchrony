...
LB10_rain_ymeans.w %>% 
  ggplot()+aes(NAOw, m.mean)+geom_smooth(method='lm')+geom_jitter(aes(col=Site), width=0.2)+ylab('Mean monthly rain')+xlab("Winter NAO")+geom_vline(xintercept = 0, col='grey70')
...
plot_monthlyrain_nao=
  LB10_rain_ymeans.w %>% 
  ggplot()+aes(NAOw, m.mean)+geom_smooth(method='lm')+geom_jitter(aes(col=Site), width=0.2)+ylab('Mean monthly winter rain')+xlab("Winter NAO")+geom_vline(xintercept = 0, col='grey70')
...
times=time.lb10, clev=1)$cdat, time.lb10), title='WT monthly rain LI8'
)
...
# Plot of wavelet monthly rain LI8
```{r}
...
LB10_rain_ymeans.w %>% 
  ...
  select(c(Site, Ante, m.mean)) %>% 
  pivot_wider(names_from = Ante, values_from = m.mean) %>% filter(Site=='LI8') %>% select(-'Site') %>% as.matrix(),
times=time.lb10, clev=4)$cdat, time.lb10), title='Mean monthly winter rain LI8'
)
...
# Plot both winter Temp and Rain vs NAO
```{r}
library(patchwork)
...
plot_winterT_nao + plot_monthlyrain_nao
...
*some evident sign that positive nao phases are linked to higher PC1 variance*
```{r}
...
# this is also like *LB10* in Ecology
```{r}
...
#function that does the pop synch for each spp and rbindlist the list
```{r}
...
m1=glm(mean.pop.synch~geodist*NAOmw, data=meanpop_synchronies_mw, family='gaussian')
summary(m1)

```

# simple models for different quantiles of NAO with differences in both slope and intercept
```{r}
m0=glm(mean.pop.synch~geodist, data= meanpop_synchronies_mw %>% filter(NAOmw.q5 =="[-0.142,0.0639]"))

m7=glm(mean.pop.synch~geodist, data=meanpop_synchronies_mw %>% filter(NAOmw.q5 =="(0.636,1.02]"))
...
*some support for the inclusion of a random slope effects, not only intercept for the different quatiles* 
```{r}

m0=
  lme(mean.pop.synch ~ geodist ,  data=meanpop_synchronies_mw)

m1=
  lme(mean.pop.synch ~ geodist , random=~1|NAOmw.q5, data=meanpop_synchronies_mw)

m2=
  lme(mean.pop.synch ~ geodist , random=~1+geodist|NAOmw.q5, data=meanpop_synchronies_mw)
...
abund.synch_mw %>% 
  #mutate(RegRich.mw= reg.richness %>% rollmean(k=6, align = 'center')) %>% 
  #mutate(MeanRich.mw= rowMeans(Richness.mw)) %>% 
  ggplot()+aes(wNAOwindow, Abund.synch.mw, label=row.names(abund.synch_mw))+geom_point(size=2)+geom_text_repel(size=3)+
  geom_smooth( col='grey50', alpha=.3, method='lm')+ylab('Abundance synchrony mw')+ xlab('Winter NAO mw')
...
wang_mw[c(1,3,4,9)] %>% 
  ...
  scale_color_manual(values=c("orange", 'forestgreen', 'grey20'), labels=c('CV population', 'CV metapopulation', 'CV metacommunity'))+theme(legend.title = element_blank())+ylab('Variability')+xlab('Winter NAO window')
...
### Must include the functional trait part
*trait analyses*
...
Rao_dev.mw %>%  pivot_longer(cols=c(1:10), values_to = 'Rao_deviation', names_to = 'Code')) %>% 
  ...
  theme(axis.title = element_text(size=12))+scale_color_discrete(name='Stream')+xlab('Functional deviation')+geom_smooth(aes(col=name), se=F, method='lm', size=0.2)
```
```{r}
library(nlme)
```
